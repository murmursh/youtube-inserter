...
class Patcher(object):
    ...
    def __init__(self, executable_path=None, force=False, version_main: int = 0):
        ...
        if os.path.isfile(self.executable_path) and os.path.exists(self.executable_path):
            self._custom_exe_path = True
    ...
    def unzip_package(self, fp):
        ...
        try:
            ...
            h, t = os.path.split(self.executable_path)
            timestamp = time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())
            t = f"{timestamp}{t}"
            self.executable_path = os.path.join(h, t)
            os.rename(os.path.join(self.zip_path, self.exe_name), self.executable_path)
